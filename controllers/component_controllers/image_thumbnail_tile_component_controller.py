...
from controllers.component_controllers_helpers.rename_image_helper import rename_image, update_tab_images_map
...
class ImageThumbnailTileComponentController(Controller):
    ...
    def handle_rename_image(self, command):
        image = command.__dict__.get(ChangeImageNameConstants.IMAGE_MODEL.value)
        tab_images_map = AppStateService().get_state(AppStateConstants.TAB_IMAGES_MAP.value)

        rename_image(image, unique_filename=True)
        updated_tab_images_map = update_tab_images_map(image, tab_images_map)
